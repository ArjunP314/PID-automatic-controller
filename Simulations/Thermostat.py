...
class Thermostat(Simulator):
  ...
  def __init__(self, initial_temp=20.0, ambient_temp=20.0, heater_temp=35.0, heat_coefficient=1.0, entropy=0.2):
    ...
    Arguments:
      ...
      heat_coefficient {float} -- How easily the air can be heated by the heater.
      entropy {float} -- Ranges from 0 to 1. The maximum probability the air flowcan vary from what the user sets it to.
    """
    super().__init__(0.0, 1.0)
    self._temp = initial_temp
    self._ambient_temp = ambient_temp
    self._heater_temp = heater_temp
    self._heater_coefficient = heat_coefficient
  ...
  def step(self, dt: float):
    ...
    delta_temp = hot_delta_temp + ambient delta_temp
