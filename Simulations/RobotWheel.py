...
class RobotWheel(Simulator):
  """Simulates a wheel (or pair of wheel on a robot that respond to a certain voltage (ranging from -1 to 1)."""

  def __init__(self, responsiveness=1.0, entropy=0.05, friction=0.05):
    ...
    Arguments:
      ...
      friction {float} -- A coefficient of friction (where the friction is proportional to the velocity squared for this Simulator)
    """
  ...
  def set_input(self, val: float) -> float:
    ...
    """
    #Add in a bit or randomness to the system
  ...
  def _get_force(self) -> float:
    """Converts the voltage set on the wheel to a translational force applid on RobotWheel body.

    Returns: 
      float -- The force applid on the RobotWheel body. 
    """
    ...
    #Arbitrary function for conversion between voltage and force
    force = (voltage*self.)responsiveness)**3
